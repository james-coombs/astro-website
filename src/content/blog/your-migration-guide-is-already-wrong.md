...
One more, and it happened three times while I was writing this.

The paragraph above first said the curated list fell to 6. That was the number on the branch where the fix had been written. By the time the pull request actually went up, the same field read 4, because two more of those files had been migrated in the weeks in between. I had taken a count from a document describing the codebase instead of from the codebase, which is the precise error this article is about, committed inside the article about it.

Then I corrected a second number in the same paragraph, and I got it from a commit message. It said two files still reached the old framework through a sibling style module. The answer was three. The commit message was accurate about the state of the branch when it was written and wrong about the state that shipped, and I had read it instead of the tree, which is the same error I had just finished confessing to, two sentences away from the confession.

Then the count moved again on its own. The follow-up work that re-curated the list raised it from 4 to 7, after the piece was already published. That one is not carelessness. A count is not wrong because somebody was sloppy with it. It is wrong because it has a date and the document does not.

The fix is the same one all three times, and it is the one I am recommending to you: I ran the thing and read what it said.
